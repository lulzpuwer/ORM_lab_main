...
def problem_two(request):
  instructor = Instructor.objects.filter(hire_date__lte = '2010-01-01').order_by('hire_date')

  for instructorss in instructor:
    print(f'First Name: {instructorss.frist_name} Last Name: {instructorss.last_name} Date of hire{instructorss.hire_date}')
...
def problem_three(request):
  instructor = Instructor.objects.filter(instructor_id=2)
  cour = Course.objects.filter(instructor_id=2)
...
def problem_four(request):
  stud_count = Student.objects.filter(id__gt=0).count()
  cor_cnt = Course.objects.filter(id__gt=0).count()
  instr_cnt = Instructor.objects.filter(id__gt=0).count()

  print(f'student count: {stud_count}, instructor count: {instr_cnt}, course count: {cor_cnt}')
  return complete(request)
